...
- current retrieval prototypes are DataFusion-native marker UDFs on the prepared session surface:
  ...
  - coordinated score composition through `qdrant_formula_score(...)` and `qdrant_fusion_score(...)`
    ...
    - `qdrant_fusion_score(...)` now additionally admits a local `RRF` fallback over aligned `INNER` / `LEFT` / `RIGHT` joins on `id` with explicit score-column inputs; `DBSF` remains on the exact coordinated remote path, and `CROSS JOIN` stays unsupported by design because it does not align the same candidate across branches
