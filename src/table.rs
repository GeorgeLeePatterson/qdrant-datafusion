...
{}",
    ...
    #[test]
    fn physical_plan_uses_qdrant_count_exec_through_non_null_literal_alias_subquery() {
        let provider = rank_exact_provider(Schema::new(vec![
            Field::new(ID_FIELD_NAME, DataType::Utf8, false),
            Field::new(PAYLOAD_FIELD_NAME, DataType::Utf8, true),
        ]));
    ...
    #[test]
    fn physical_plan_uses_qdrant_facet_exec_through_payload_and_literal_alias_subquery() {
        let provider = QdrantTableProvider {
            payload_schema: payload_schema([(
                "tag",
                PayloadSchemaInfo {
    ...
    #[test]
    fn physical_plan_uses_qdrant_facet_exec_through_payload_alias_subquery() {
        let provider = QdrantTableProvider {
            payload_schema: payload_schema([(
                "tag",
                PayloadSchemaInfo {
    ...
    #[test]
    fn physical_plan_keeps_local_aggregate_shell_above_nearest_query_kernel() {
        let provider = test_provider(Schema::new(vec![
            Field::new(ID_FIELD_NAME, DataType::Utf8, false),
            Field::new(PAYLOAD_FIELD_NAME, DataType::Utf8, true),
    ...
    #[test]
    fn physical_plan_builds_qdrant_insert_sink_for_replace() {
        let batch = dense_insert_batch();
        let schema = batch.schema();
        let provider = QdrantTableProvider::new_for_planner(
            "vectors".to_owned(),
        ...
        let dataframe = ctx
            .sql("REPLACE INTO vectors SELECT id, payload, vector FROM staging")
            .now_or_never()
            .expect("sql future is ready")
            .expect("insert dataframe");
        let plan = dataframe
    ...
    #[test]
    fn physical_plan_composes_local_rrf_fusion_over_inner_join() {
        let provider = test_provider(Schema::new(vec![
            Field::new(ID_FIELD_NAME, DataType::Utf8, false),
            Field::new(
                "embedding",
        ...
        let dataframe = ctx
            .sql(
                "SELECT dense.id AS id, qdrant_fusion_score('RRF', dense.score, sparse.score) AS \
                 score FROM (SELECT id, qdrant_nearest_score(embedding, 1.0, 0.0) AS score FROM \
                 vectors ORDER BY score DESC LIMIT 5) dense JOIN (SELECT id, \
                 qdrant_nearest_score(aux, 0.0, 1.0) AS score FROM vectors ORDER BY score DESC \
                 LIMIT 5) sparse ON dense.id = sparse.id ORDER BY score DESC LIMIT 2",
            )
            .now_or_never()
            .expect("sql future is ready")
            .expect("dataframe");
    ...
    #[test]
    fn physical_plan_composes_local_dbsf_fusion_over_inner_join() {
        let provider = test_provider(Schema::new(vec![
            Field::new(ID_FIELD_NAME, DataType::Utf8, false),
            Field::new(
                "embedding",
        ...
        let dataframe = ctx
            .sql(
                "SELECT dense.id AS id, qdrant_fusion_score('DBSF', dense.score, sparse.score) AS \
                 score FROM (SELECT id, qdrant_nearest_score(embedding, 1.0, 0.0) AS score FROM \
                 vectors ORDER BY score DESC LIMIT 5) dense JOIN (SELECT id, \
                 qdrant_nearest_score(aux, 0.0, 1.0) AS score FROM vectors ORDER BY score DESC \
                 LIMIT 5) sparse ON dense.id = sparse.id ORDER BY score DESC LIMIT 2",
