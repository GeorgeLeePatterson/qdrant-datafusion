...
pub const PAYLOAD_IS_MISSING_FUNCTION_NAME: &str = "payload_is_missing";
pub const PAYLOAD_IS_NULL_FUNCTION_NAME: &str = "payload_is_null";
pub const PAYLOAD_IS_EMPTY_FUNCTION_NAME: &str = "payload_is_empty";
pub const PAYLOAD_HAS_VALUES_FUNCTION_NAME: &str = "payload_has_values";
...
pub(crate) const PAYLOAD_IS_MISSING_ACCESS_FUNCTION_NAME: &str =
    "__qdrant_payload_is_missing_access";
pub(crate) const PAYLOAD_IS_NULL_ACCESS_FUNCTION_NAME: &str = "__qdrant_payload_is_null_access";
pub(crate) const PAYLOAD_IS_EMPTY_ACCESS_FUNCTION_NAME: &str = "__qdrant_payload_is_empty_access";
pub(crate) const PAYLOAD_HAS_VALUES_ACCESS_FUNCTION_NAME: &str =
    "__qdrant_payload_has_values_access";
...
const PAYLOAD_IS_MISSING_ALIASES: &[&str] = &["qdrant_payload_is_missing"];
const PAYLOAD_IS_NULL_ALIASES: &[&str] = &["qdrant_payload_is_null"];
const PAYLOAD_IS_EMPTY_ALIASES: &[&str] = &["qdrant_payload_is_empty"];
const PAYLOAD_HAS_VALUES_ALIASES: &[&str] = &["qdrant_payload_has_values"];
...
pub(crate) fn is_payload_is_missing_function_name(name: &str) -> bool {
    name == PAYLOAD_IS_MISSING_FUNCTION_NAME || PAYLOAD_IS_MISSING_ALIASES.contains(&name)
}

pub(crate) fn is_payload_is_null_function_name(name: &str) -> bool {
    name == PAYLOAD_IS_NULL_FUNCTION_NAME || PAYLOAD_IS_NULL_ALIASES.contains(&name)
}
...
pub(crate) fn is_payload_has_values_function_name(name: &str) -> bool {
    name == PAYLOAD_HAS_VALUES_FUNCTION_NAME || PAYLOAD_HAS_VALUES_ALIASES.contains(&name)
}
...
#[must_use]
pub fn qdrant_payload_is_missing(accessor: Expr) -> Expr {
    qdrant_payload_is_missing_udf().call(vec![accessor])
}

#[must_use]
pub fn qdrant_payload_is_null(accessor: Expr) -> Expr {
    qdrant_payload_is_null_udf().call(vec![accessor])
}
...
#[must_use]
pub fn qdrant_payload_has_values(accessor: Expr) -> Expr {
    qdrant_payload_has_values_udf().call(vec![accessor])
}
...
pub(crate) fn qdrant_payload_is_missing_udf() -> ScalarUDF {
    static UDF: OnceLock<ScalarUDF> = OnceLock::new();
    UDF.get_or_init(|| ScalarUDF::new_from_impl(PayloadPredicateUdf::is_missing())).clone()
}

pub(crate) fn qdrant_payload_is_null_udf() -> ScalarUDF {
    static UDF: OnceLock<ScalarUDF> = OnceLock::new();
    UDF.get_or_init(|| ScalarUDF::new_from_impl(PayloadPredicateUdf::is_null())).clone()
}

pub(crate) fn qdrant_payload_has_values_udf() -> ScalarUDF {
    static UDF: OnceLock<ScalarUDF> = OnceLock::new();
    UDF.get_or_init(|| ScalarUDF::new_from_impl(PayloadPredicateUdf::has_values())).clone()
}
...
pub(crate) fn qdrant_payload_is_missing_access_udf() -> ScalarUDF {
    static UDF: OnceLock<ScalarUDF> = OnceLock::new();
    UDF.get_or_init(|| {
        ScalarUDF::new_from_impl(PayloadPredicateAccessUdf::new(PayloadPredicateKind::IsMissing))
    })
    .clone()
}

pub(crate) fn qdrant_payload_is_null_access_udf() -> ScalarUDF {
    static UDF: OnceLock<ScalarUDF> = OnceLock::new();
    UDF.get_or_init(|| {
        ScalarUDF::new_from_impl(PayloadPredicateAccessUdf::new(PayloadPredicateKind::IsNull))
    })
    .clone()
}

pub(crate) fn qdrant_payload_has_values_access_udf() -> ScalarUDF {
    static UDF: OnceLock<ScalarUDF> = OnceLock::new();
    UDF.get_or_init(|| {
        ScalarUDF::new_from_impl(PayloadPredicateAccessUdf::new(PayloadPredicateKind::HasValues))
    })
    .clone()
}
...
impl PayloadPredicateKind {
    fn function_name(self) -> &'static str {
        match self {
            ...
            Self::IsMissing => PAYLOAD_IS_MISSING_FUNCTION_NAME,
            Self::IsNull => PAYLOAD_IS_NULL_FUNCTION_NAME,
            Self::IsEmpty => PAYLOAD_IS_EMPTY_FUNCTION_NAME,
            Self::HasValues => PAYLOAD_HAS_VALUES_FUNCTION_NAME,
    ...
    fn aliases(self) -> &'static [&'static str] {
        match self {
            ...
            Self::IsMissing => PAYLOAD_IS_MISSING_ALIASES,
            Self::IsNull => PAYLOAD_IS_NULL_ALIASES,
            Self::IsEmpty => PAYLOAD_IS_EMPTY_ALIASES,
            Self::HasValues => PAYLOAD_HAS_VALUES_ALIASES,
    ...
    fn internal_function_name(self) -> &'static str {
        match self {
            ...
            Self::IsMissing => PAYLOAD_IS_MISSING_ACCESS_FUNCTION_NAME,
            Self::IsNull => PAYLOAD_IS_NULL_ACCESS_FUNCTION_NAME,
            Self::IsEmpty => PAYLOAD_IS_EMPTY_ACCESS_FUNCTION_NAME,
            Self::HasValues => PAYLOAD_HAS_VALUES_ACCESS_FUNCTION_NAME,
    ...
    fn return_type(self) -> DataType {
        match self {
            Self::Exists | Self::IsMissing | Self::IsNull | Self::IsEmpty | Self::HasValues => {
                DataType::Boolean
            }
...
impl PayloadPredicateUdf {
    ...
    fn is_missing() -> Self { Self::new(PayloadPredicateKind::IsMissing) }

    fn is_null() -> Self { Self::new(PayloadPredicateKind::IsNull) }

    fn is_empty() -> Self { Self::new(PayloadPredicateKind::IsEmpty) }

    fn has_values() -> Self { Self::new(PayloadPredicateKind::HasValues) }
...
impl ScalarUDFImpl for PayloadPredicateUdf {
    ...
    fn simplify(&self, args: Vec<Expr>, _info: &SimplifyContext) -> Result<ExprSimplifyResult> {
        ...
        let expr = match self.kind {
            ...
            PayloadPredicateKind::IsMissing => {
                qdrant_payload_is_missing_access_udf().call(vec![payload, lit(path)])
            }
            PayloadPredicateKind::IsNull => {
                qdrant_payload_is_null_access_udf().call(vec![payload, lit(path)])
            }
            PayloadPredicateKind::IsEmpty => {
                qdrant_payload_is_empty_access_udf().call(vec![payload, lit(path)])
            }
            PayloadPredicateKind::HasValues => {
                qdrant_payload_has_values_access_udf().call(vec![payload, lit(path)])
            }
...
fn payload_predicate_scalar(
    ...
    Ok(match kind {
        ...
        PayloadPredicateKind::IsMissing => ScalarValue::Boolean(Some(value.is_none())),
        PayloadPredicateKind::IsNull => {
            ScalarValue::Boolean(Some(matches!(value, Some(serde_json::Value::Null))))
        }
        PayloadPredicateKind::IsEmpty => ScalarValue::Boolean(Some(payload_is_empty_value(value))),
        PayloadPredicateKind::HasValues => {
            ScalarValue::Boolean(Some(payload_has_values_value(value)))
        }
...
fn payload_has_values_value(value: Option<serde_json::Value>) -> bool {
    payload_values_count_value(value).is_some_and(|count| count > 0)
}
...
mod tests {
    ...
    #[test]
    fn payload_is_missing_matches_missing_only() {
        assert_eq!(
            payload_predicate_scalar(
                PayloadPredicateKind::IsMissing,
                Some(r#"{"list": []}"#),
    ...
    #[test]
    fn payload_is_null_matches_explicit_null_only() {
        assert_eq!(
            payload_predicate_scalar(
                PayloadPredicateKind::IsNull,
                Some(r#"{"list": []}"#),
    ...
    #[test]
    fn payload_has_values_matches_positive_cardinality_only() {
        assert_eq!(
            payload_predicate_scalar(
                PayloadPredicateKind::HasValues,
                Some(r#"{"list": []}"#),
