...
| Scan payload projection typing | `src/context/expr_planner.rs`, `src/analyzer/state/source.rs`, `src/expr_fn/payload.rs`, `src/expr_fn/payload_access.rs` | `Local fallback` | Known `payload:<path>` projections and raw arithmetic / scalar-function compositions now type during SQL planning from authoritative payload-schema metadata, so direct, subquery, CTE, union, ordering, window, and aggregate shells localize cleanly when they leave exact qdrant ownership. The current exact count/facet kernels now also survive benign payload-alias projection shells, and exact row-count kernels additionally survive benign non-null `id` alias shells, when later aggregate semantics still match the admitted exact contracts. Remaining unsupported cases in this family are broader SQL-shape limits such as non-lateral `UNNEST`, not missing payload typing. | Keep payload typing anchored to schema metadata when new SQL-planning seams are added, and widen exact shell preservation only when the aggregate contract remains provably unchanged. |
...
| Canonical delete semantics | `src/table/provider.rs`, `src/table/delete.rs` | `Exact only` | Provider-table `DELETE FROM ...` now lowers only when the predicate can be expressed through the existing qdrant filter algebra. The provider counts exact matches first and then deletes through the same filter, while empty `DELETE FROM vectors` deletes all rows through that same exact boundary. Delete predicates that would require local residual execution stay in the unsupported inventory instead of being approximated. | Broaden toward `Exact + residual` only when local delete semantics are explicit and mirrored in the SQL catalog. |
| Canonical row-rewrite update semantics | `src/table/provider.rs`, `src/table/update.rs`, `src/arrow/serialize.rs` | `Exact + residual` | Provider-table `UPDATE` now executes as a stable-id row rewrite over the canonical provider schema. Exact admitted qdrant filters lower remotely, non-pushdownable predicates stay local over the materialized candidate rows, and matched rows are rewritten through the same canonical row serializer before `upsert`. The current mirrored SQL inventory covers payload rewrites, `CASE` assignment, `NULL`, and whole-table update; `id` assignment stays deferred and `UPDATE ... FROM` remains upstream unsupported. | Broaden deliberately: stable-id mutation, backend-specific partial payload/vector mutation APIs, and broader joined update forms need explicit contract work beyond the current `Q-069` row-rewrite checkpoint. |
