...
| Query-kernel projection handling | `src/analyzer/op.rs`, `src/analyzer/state/local.rs` | `Exact + residual` | Remote retrieval kernels can now leave benign local projection shells, including later score projection above residual local filters. | Keep broadening only through explicit local-shell rewrites rather than shape-specific fatal paths. |
