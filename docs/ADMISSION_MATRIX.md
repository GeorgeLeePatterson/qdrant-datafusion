...
| Canonical write semantics | `src/table/provider.rs`, `src/table/insert.rs`, `src/arrow/serialize.rs` | `Strict for now` | The admitted write contract is now canonical-schema `INSERT INTO` plus `INSERT OVERWRITE`. Explicit target-column inserts are part of that contract when `DataFusion` normalizes reordered columns or omitted nullable `payload` back into the canonical schema. On the current qdrant runtime line, `INSERT INTO` uses verified `InsertOnly` behavior, preserving existing ids while inserting new ids; `INSERT OVERWRITE` clears the collection first, then writes canonical rows through the same sink path. Remaining noncanonical shapes and `REPLACE` stay in the unsupported inventory as `InvalidInput` or `Deferred`, not hidden sink coercions. | Broaden only through explicit write-semantics work, not by silently coercing arbitrary insert schemas at the sink boundary. |
