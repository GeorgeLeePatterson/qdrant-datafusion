...
| Canonical write semantics | `src/table/provider.rs`, `src/table/insert.rs`, `src/table/delete.rs`, `src/arrow/serialize.rs` | `Strict for now` | The admitted mutation contract is now canonical-schema `INSERT INTO`, `REPLACE INTO`, `INSERT OVERWRITE`, plus exact `DELETE FROM ...` over the provider table. Explicit target-column inserts are part of the insert/replace/overwrite contract when `DataFusion` normalizes reordered columns or omitted nullable `payload` back into the canonical schema. On the current qdrant runtime line, `INSERT INTO` uses verified `InsertOnly` behavior, preserving existing ids while inserting new ids; `REPLACE INTO` uses verified `Upsert` behavior, replacing colliding ids while inserting new ids; `INSERT OVERWRITE` clears the collection first, then writes canonical rows through the same sink path; `DELETE` counts exact matches first and then deletes through the same exact qdrant filter algebra already used by scan pushdown. Remaining noncanonical shapes, residual/local delete semantics, and broader update/merge semantics stay in the unsupported inventory as `InvalidInput`, `Upstream`, or `Deferred`, not hidden sink coercions. | Broaden only through explicit write-semantics work, not by silently coercing arbitrary insert schemas or delete predicates at the provider boundary. |
