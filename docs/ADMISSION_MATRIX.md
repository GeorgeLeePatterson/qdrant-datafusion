...
| Append-only writes | `src/table/provider.rs`, `src/table/insert.rs`, `src/arrow/serialize.rs` | `Strict for now` | The admitted write contract is still append-only into the canonical qdrant provider schema. Explicit target-column inserts are part of that contract when `DataFusion` normalizes reordered columns or omitted nullable `payload` back into the canonical schema. Remaining noncanonical append shapes without that normalization stay in the unsupported inventory as `InvalidInput`, not `Deferred`. | Broaden only through explicit write-semantics work, not by silently coercing arbitrary insert schemas at the sink boundary. |
