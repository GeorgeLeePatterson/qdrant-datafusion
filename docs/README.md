...
4. `tests/catalog/mod.rs`: mirrored supported / unsupported SQL inventories consumed by the integration suites, including explicit subquery coverage per namespace, broader SQL syntax-family inventory, explicit unsupported classifications (`Deferred`, `ByDesign`, `Upstream`, `InvalidInput`), and the current supported/deferred mutation inventory across `INSERT INTO`, `REPLACE INTO`, `INSERT OVERWRITE`, `DELETE`, and row-rewrite `UPDATE`. The catalog should be expanded from the SQL space outward, not only from already-known code gaps, and queries should move from `unsupported` to `supported` as soon as behavior widens rather than being left behind as stale inventory.
