...
39. `Q-041`: `INSERT INTO` now lands on a deliberate canonical write contract.
    ...
    - the current admitted contract is explicit: `DataFusion` must present the canonical qdrant table schema at the sink boundary
...
51. `Q-054`: the supported / unsupported SQL catalogs now act as the namespace inventory rather than a thin sample set.
    ...
    - the mirrored SQL inventory now includes representative `INNER`, `LEFT`, `RIGHT`, `FULL`, `CROSS`, `SEMI`, and `ANTI` join forms plus `ON` / `USING` variants across scan, writes, query-kernel local join shells, and coordinated qdrant shapes
...
54. `Q-057`: explicit payload-existence semantics are now admitted separately from SQL null semantics.
    - `payload_exists(payload:<path>)` now executes locally and lowers exactly to Qdrant field-presence predicates on scan filters
    - the contract is explicit: present explicit `null` and empty arrays still count as existing fields
    - catalog coverage now tracks direct and subquery existence-filter SQL in the supported inventory
55. `Q-020`: the explicit payload presence/null/missing/empty/non-empty/count family is now closed on the current contract.
    - `payload_is_missing(payload:<path>)` now executes locally and lowers exactly to the missing-field subset through the existing presence predicate contract
...
64. `Q-065`: canonical `INSERT INTO` now admits the current `DataFusion` target-column normalization path instead of underclaiming the write surface.
