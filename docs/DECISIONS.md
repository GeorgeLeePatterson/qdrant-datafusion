...
30. The first admitted retrieval prototype is nearest-neighbor query through a DataFusion-native
    ...
    - current admitted scope is:
      ...
      - benign local projection shells, local aggregate shells, local window shells, and local residual filter shells can remain above the closed qdrant query kernel, including later score projection above those local filter shells
