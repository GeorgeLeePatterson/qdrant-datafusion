...
| Mutation | collection-wide pre-clear | `delete_points` | canonical-schema `INSERT OVERWRITE` | `Current` | `INSERT OVERWRITE` now clears the collection first through `delete_points(Filter::default())`, then writes canonical rows through the same insert sink. This is not a standalone SQL `DELETE` surface yet. |
| Mutation | point delete | `delete_points` | `DELETE` | `Later` | Standalone `DELETE` SQL is still deferred even though `INSERT OVERWRITE` now uses collection-wide delete internally. |
