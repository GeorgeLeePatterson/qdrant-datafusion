...
| Row production | payload-key order-by query | `query(Query::OrderBy)` | relation-producing retrieval | `Current` | Now exposed through `qdrant_order_by_score(payload:<path>[, direction])` on the prepared session surface. Exact lowering currently admits canonical indexed integer / float / datetime payload paths plus order-preserving casts, with direction accepted as boolean or `'asc'` / `'desc'` literals. When SQL omits `LIMIT`, the remote request uses Qdrant's default result count. The current mirrored SQL inventory closes descending remote order through `ORDER BY score DESC`; non-canonical scalar expressions remain explicitly unsupported. |
...
| Aggregation / grouping | point count | `count` | exact row-count-preserving `COUNT(...)` pushdown | `Current` | The first aggregate-like slice is now admitted through a narrow analyzer / extension-planner path over a single `Qdrant` source. It composes directly over the existing predicate algebra. The current exact row-count forms are `COUNT(*)`, non-null literals such as `COUNT(1)`, and non-null `id` column references, including benign `id` alias shells, and benign non-null literal alias shells can now preserve that same exact contract through subquery / `CTE` boundaries. |
| Aggregation / grouping | top-facet grouped counts over one scalar payload field | `facet` | `GROUP BY payload:<path> ... LIMIT N` | `Current` | The current facet slice now admits keyword, bool, and lookup-capable integer payload indexes. Projected `ORDER BY count DESC` is optional and redundant because it matches qdrant facet's native top-count order. Grouped exact counts currently use the same row-count-preserving `COUNT(...)` subset as exact point count. Facet key outputs now follow the authoritative payload scalar type on the current runtime line, and benign payload-alias plus non-null literal alias subquery / `CTE` shells can still close exactly on the same admitted top-facet contract, so this remains intentionally narrower than general SQL grouping without forcing everything back to textual keys. Live collection introspection on the current runtime line now preserves integer lookup/range metadata well enough to keep integer facets exact on the same admission contract. |
| Aggregation / grouping | grouped search results | `query_groups`, `search_groups`, `recommend_groups` | grouped retrieval relation | `Current` | The current admitted grouped retrieval slice is narrow but now query-family rather than nearest-only: top-1 grouped retrieval through `SELECT DISTINCT ON (payload:<path>) ... <grouped-score-surface> ... ORDER BY payload:<path>[ DESC]`, with optional trailing `, score DESC` as an explicit in-group tie-break, lowering to `query_groups` with group size 1 over one scalar keyword or lookup-capable integer payload field. Current grouped score surfaces are `qdrant_nearest_score(...)`, `qdrant_sample_score(...)`, `qdrant_recommend_score(...)`, `qdrant_discover_score(...)`, `qdrant_context_score(...)`, `qdrant_nearest_with_mmr_score(...)`, and `qdrant_relevance_feedback_score(...)`. Grouped execution validates that returned group ids match scalar payload values on the hits, and because SQL group ordering is finalized locally, outer `LIMIT` remains local instead of being pushed into `query_groups`. Broader grouped `DISTINCT ON` SQL now localizes above a normal query kernel instead of failing, but broader exact grouped retrieval variants remain later. |
...
| Mutation | point upsert | `upsert_points`, `upsert_points_chunked` | canonical-schema `REPLACE INTO` | `Current` | Current write support is narrow and deliberate: `QdrantTableProvider::insert_into` lowers canonical-schema `REPLACE INTO` through `DataSinkExec` when `DataFusion` produces the canonical qdrant table schema. Explicit target-column inserts are already part of that contract when `DataFusion` normalizes reordered columns or omitted nullable `payload` back into the canonical schema. On the validated current runtime line, `REPLACE INTO` uses qdrant's `Upsert` behavior, replacing colliding ids while inserting new ids. |
...
| Mutation | point delete | `count`, `scroll`, `delete_points` | canonical `DELETE` | `Current` | Canonical provider-table `DELETE FROM ...` now lowers through `QdrantTableProvider::delete_from`. Exact admitted filters still lower through the existing qdrant filter algebra, residual predicates now evaluate locally over scrolled candidate rows, and the matched ids are then deleted through `delete_points`. Empty `DELETE FROM vectors` is admitted and deletes all rows. |
| Mutation | provider-table row rewrite update | `scroll`, `get_points`, `delete_points`, `upsert_points` | canonical row-rewrite `UPDATE` | `Current` | `QdrantTableProvider::update` now executes provider-table `UPDATE` as a canonical row rewrite on the schema. Exact admitted filters lower through the existing qdrant filter algebra, residual predicates stay local over scrolled candidate rows, and matched rows are rewritten through `upsert_points`. `id` assignment is now admitted when the rewritten final ids remain unique within the update set and do not collide with untouched existing rows; `UPDATE ... FROM` is still upstream unsupported. |
...
2. count
3. facet
4. nearest / sample / recommend / discover / context / nearest-with-MMR / relevance feedback
5. grouped retrieval
