...
| SQL-native `Qdrant` capability surface | search / recommend / discover / fusion / grouped query forms | Partial | The first SQL-facing retrieval prototypes now exist through `qdrant_nearest_score(...)` and `qdrant_sample_score(...)`, but broader stable SQL recommend / discover / fusion / grouped-query semantics are still intentionally deferred. |
