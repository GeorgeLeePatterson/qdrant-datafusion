...
7. Canonical-schema `INSERT INTO`, `REPLACE INTO`, `INSERT OVERWRITE`, canonical `DELETE`, and canonical row-rewrite `UPDATE` are now supported on the qdrant table schema.
...
9. Write-side Arrow/Qdrant serialization plus provider-owned delete and row-rewrite update now exist for the canonical provider schema.
   ...
   - `DELETE FROM ...` now lowers through `QdrantDeleteExec` with exact admitted qdrant filters lowered remotely and residual predicates evaluated locally over candidate rows before deletion
   - `UPDATE ... SET ...` now lowers through `QdrantUpdateExec` as a canonical row rewrite over the provider schema
   - exact admitted qdrant filters lower remotely on that update path while non-pushdownable predicates stay as local residual filters over the materialized candidate rows before rewrite, and `id` assignment is admitted when rewritten final ids remain unique and do not collide with untouched rows
...
8. `src/table/insert.rs`, `src/table/delete.rs`, `src/table/update.rs`
   - canonical `INSERT INTO` / `REPLACE INTO` / `INSERT OVERWRITE` sink implementation plus exact `DELETE` execution and row-rewrite `UPDATE` execution over the canonical provider table
9. `tests/e2e.rs`
